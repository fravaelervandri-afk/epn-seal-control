...
import { PlusCircle, Download, X, Type, Plus, Info, Loader2, Trash2, ShieldCheck, Printer } from 'lucide-react';
...
const Generator = ({ session }) => {
  ...
  // Cek jika user datang dari halaman Histori (menekan tombol "Lihat")
  useEffect(() => {
    try {
      ...
      if (storedBatchId && generateHistory.length > 0) {
        ...
        if (batch) {
          const uniqueIds = [...new Set((batch.items || []).map(item => item?.id).filter(Boolean))];
          setSelectedBatchIds(uniqueIds);
  ...
  // Auto-hitung nomor selanjutnya
  useEffect(() => {
    if (!previewBatchId && generateHistory.length > 0) {
  ...
  // --- FUNGSI DRAG & RESIZE ---
  const addQrPosition = () => setQrPositions([...qrPositions, { id: Date.now(), x: 50, y: 50, size: 20 }]);
  const removeQrPosition = (id) => qrPositions.length > 1 && setQrPositions(qrPositions.filter(pos => pos.id !== id)); 
  const addTextPosition = () => setTextPositions([...textPositions, { id: Date.now(), x: 50, y: 80, size: 4 }]);
  const removeTextPosition = (id) => setTextPositions(textPositions.filter(pos => pos.id !== id));
  ...
  const handleResizeStart = (e, dirX, index) => {
    ...
    const onMove = (mv) => {
      ...
      setQrPositions(prev => prev.map((p, i) => i === index ? { ...p, size: Math.max(5, Math.min(100, startSize + deltaPercent)) } : p));
    };
  ...
  const handleTextResizeStart = (e, dirX, index) => {
    ...
    const onMove = (mv) => {
      ...
      setTextPositions(prev => prev.map((p, i) => i === index ? { ...p, size: Math.max(0.5, Math.min(100, startSize + deltaPercent)) } : p));
    };
  ...
  const handleGenerateQR = async () => {
    ...
    let jsPDF;
    try {
        const jspdfModule = await import('jspdf'); // Panggil library lokal
    ...
    try {
      ...
      if (!previewBatchId) {
         ...
         const { error } = await supabase.from('generate_history').insert([newBatch]);
      ...
      const generateQROnCanvas = async (text, payload, size, showText) => {
          return new Promise((resolve, reject) => {
              ...
              img.onload = () => {
                  ...
                  if (showText) {
                      const fontSize = Math.floor(size * 0.09); ctx.font = `${fontSize}px monospace`;
                      const textWidth = ctx.measureText(text).width; const paddingX = Math.floor(size * 0.03); const paddingY = Math.floor(size * 0.03);
                      ctx.fillStyle = 'white'; ctx.fillRect(size - textWidth - (paddingX * 2), size - fontSize - (paddingY * 2), textWidth + (paddingX * 2), fontSize + (paddingY * 2));
                      ctx.fillStyle = 'black'; ctx.textAlign = 'right'; ctx.textBaseline = 'bottom';
      ...
      for (let i = 0; i < generatedQRs.length; i++) {
        ...
        if (printConfig.showOutline) {
           pdf.setDrawColor(200, 200, 200); pdf.setLineWidth(0.2); pdf.rect(x, y, itemWidth, itemHeight);
        }
        ...
        if (textPositions.length > 0 && generatedQRs[i]) {
            ...
            textPositions.forEach(pos => {
                ...
                const pdfFontSizePt = (previewWidthPx * (pos.size / 100)) * fontScaleFactor * 2.83465 * 1.15; 
             ...
             <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
               ...
               <div className="lg:col-span-4 space-y-5">
                  <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200/60">
                    ...
                    <input type="file" accept="image/*" onChange={handleImageUpload} className="block w-full text-sm text-slate-500 file:mr-4 file:py-2.5 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-bold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 cursor-pointer" />
                  </div>
                    ...
                    <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200/60 animate-in fade-in duration-300">
                      ...
                      <div className="grid grid-cols-2 gap-4 mb-4">
                        <div>
                          ...
                          <input type="text" value={inputPrefix} onChange={(e) => setInputPrefix(e.target.value)} className="w-full px-3 py-2 text-sm font-semibold border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" />
                        </div>
                        <div>
                          <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Mulai Dari</label>
                          <input type="number" value={startNum} onChange={(e) => setStartNum(parseInt(e.target.value) || 1)} className="w-full px-3 py-2 text-sm font-semibold border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" />
                        </div>
                      </div>
                      <div className="grid grid-cols-2 gap-4 mb-5">
                        <div>
                          <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Total ID</label>
                          <input type="number" min="1" value={count} onChange={(e) => setCount(parseInt(e.target.value) || 1)} className="w-full px-3 py-2 text-sm font-semibold border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" />
                        </div>
                        <div>
                          <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Salinan / ID</label>
                          <input type="number" min="1" value={copiesPerId} onChange={(e) => setCopiesPerId(Math.max(1, parseInt(e.target.value) || 1))} className="w-full px-3 py-2 text-sm font-semibold border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" />
                        </div>
                      </div>
                      <button onClick={handleBulkGenerate} className="w-full bg-[#146b99] text-white py-3 text-sm font-bold rounded-xl hover:bg-[#11577c] transition-colors flex items-center justify-center gap-2">
                  ...
                  <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200/60">
                    <h2 className="font-bold text-slate-800 flex items-center gap-2 mb-4 border-b pb-3">
                      <span className="bg-blue-100 text-blue-700 w-6 h-6 flex items-center justify-center rounded-full text-xs">3</span> Pengaturan Cetak
                    </h2>
                    ...
                    <div className="mb-4 bg-blue-50 border border-blue-100 p-3 rounded-lg">
                       ...
                       <div className="grid grid-cols-2 gap-3">
                         <div><label className="block text-[10px] font-bold text-gray-500 mb-1">Lebar (mm)</label><input type="number" value={printConfig.widthMm} onChange={(e) => setPrintConfig({...printConfig, widthMm: Number(e.target.value)})} className="w-full p-1.5 text-sm border rounded outline-blue-400" /></div>
                         <div><label className="block text-[10px] font-bold text-gray-500 mb-1">Tinggi (mm)</label><input type="number" value={printConfig.heightMm} onChange={(e) => setPrintConfig({...printConfig, heightMm: Number(e.target.value)})} className="w-full p-1.5 text-sm border rounded outline-blue-400" /></div>
                       </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-4">
                      <div>
                        ...
                        <select value={printConfig.paper} onChange={(e) => setPrintConfig({...printConfig, paper: e.target.value})} className="w-full px-3 py-2 text-sm font-semibold border border-slate-300 rounded-lg bg-slate-50 outline-none focus:border-blue-500">
                          <option value="A4">A4</option><option value="A5">A5</option><option value="A3">A3</option><option value="Letter">Letter</option>
                        </select>
                      </div>
                      <div>
                        <label className="block text-[11px] font-bold text-slate-500 uppercase mb-1">Orientasi</label>
                        <select value={printConfig.orientation} onChange={(e) => setPrintConfig({...printConfig, orientation: e.target.value})} className="w-full px-3 py-2 text-sm font-semibold border border-slate-300 rounded-lg bg-slate-50 outline-none focus:border-blue-500">
                    ...
                    <div className="mb-4">
                        ...
                        <select value={printConfig.cols} onChange={(e) => setPrintConfig({...printConfig, cols: parseInt(e.target.value)})} className="w-full px-3 py-2 text-sm font-semibold border border-slate-300 rounded-lg bg-slate-50 outline-none focus:border-blue-500">
                          <option value="1">1 Kolom</option><option value="2">2 Kolom</option><option value="3">3 Kolom</option><option value="4">4 Kolom</option><option value="5">5 Kolom</option>
                        </select>
                    </div>
                    ...
                    <div className="mb-4 border-t border-slate-200 pt-4 space-y-4">
                      <label className="flex items-start gap-3 cursor-pointer group">
                        <div className="mt-0.5">
                          <input type="checkbox" checked={printConfig.autoCenter} onChange={(e) => setPrintConfig({...printConfig, autoCenter: e.target.checked})} className="w-4 h-4 text-[#146b99] rounded border-gray-300 focus:ring-[#146b99] cursor-pointer" />
                        </div>
                        <div>
                      ...
                      <div className="mb-4 bg-slate-50 p-3 rounded-xl border border-slate-200/60">
                        ...
                        <div className="grid grid-cols-3 gap-2">
                          <div>
                            ...
                            <input type="number" min="0" value={printConfig.marginTop} onChange={(e) => setPrintConfig({...printConfig, marginTop: e.target.value})} className="w-full px-2 py-1.5 text-sm font-semibold border border-slate-300 rounded bg-white outline-none focus:border-blue-500" />
                          </div>
                          <div>
                            <label className="block text-[10px] text-slate-500 mb-1 font-semibold">Bawah</label>
                            <input type="number" min="0" value={printConfig.marginBottom} onChange={(e) => setPrintConfig({...printConfig, marginBottom: e.target.value})} className="w-full px-2 py-1.5 text-sm font-semibold border border-slate-300 rounded bg-white outline-none focus:border-blue-500" />
                          </div>
                          <div>
                            <label className="block text-[10px] text-slate-500 mb-1 font-semibold">Samping</label>
                            <input type="number" min="0" value={printConfig.marginX} onChange={(e) => setPrintConfig({...printConfig, marginX: e.target.value})} className="w-full px-2 py-1.5 text-sm font-semibold border border-slate-300 rounded bg-white outline-none focus:border-blue-500" />
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="mb-6 bg-slate-50 p-3 rounded-xl border border-slate-200/60">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                           ...
                           <input type="number" min="0" value={printConfig.gapY} onChange={(e) => setPrintConfig({...printConfig, gapY: e.target.value})} className="w-full px-2 py-1.5 text-sm font-semibold border border-slate-300 rounded bg-white outline-none focus:border-blue-500" />
                        </div>
                        <div className={printConfig.cols === 1 ? 'opacity-50' : ''}>
                           ...
                           <input type="number" min="0" value={printConfig.gapX} onChange={(e) => setPrintConfig({...printConfig, gapX: e.target.value})} disabled={printConfig.cols === 1} className="w-full px-2 py-1.5 text-sm font-semibold border border-slate-300 rounded bg-white outline-none focus:border-blue-500 disabled:bg-gray-100" />
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="mb-6 border-t border-slate-200 pt-4 space-y-4">
                      <label className="flex items-start gap-3 cursor-pointer group">
                        <div className="mt-0.5">
                          <input type="checkbox" checked={printConfig.showOutline} onChange={(e) => setPrintConfig({...printConfig, showOutline: e.target.checked})} className="w-4 h-4 text-[#146b99] rounded border-gray-300 focus:ring-[#146b99] cursor-pointer" />
                        </div>
                        <div>
                      ...
                      <label className="flex items-start gap-3 cursor-pointer group">
                        <div className="mt-0.5">
                          <input type="checkbox" checked={printConfig.embedQrText} onChange={(e) => setPrintConfig({...printConfig, embedQrText: e.target.checked})} className="w-4 h-4 text-[#146b99] rounded border-gray-300 focus:ring-[#146b99] cursor-pointer" />
                        </div>
                        <div>
                    ...
                    <button onClick={handleGenerateQR} disabled={generatedQRs.length === 0 || isGeneratingPDF} className={`w-full text-white py-3.5 text-sm font-bold rounded-xl disabled:opacity-50 transition flex items-center justify-center gap-2 ${previewBatchId ? 'bg-[#76b539] hover:bg-[#69a132]' : 'bg-[#8dc63f] hover:bg-[#7bc025]'}`}>
                      {isGeneratingPDF ? <><Loader2 size={18} className="animate-spin" /> Menyiapkan PDF...</> : previewBatchId ? <><Download size={18} /> Unduh PDF Batch</> : <><Download size={18} /> Simpan sebagai PDF</>}
                    </button>
                  </div>
               </div>

               {/* ========================================================= */}
               {/* KOLOM KANAN: PREVIEW & AREA CETAK                           */}
               {/* ========================================================= */}
               <div className="lg:col-span-8 flex flex-col gap-6">
                  {previewBatchId && (
                    <div className="bg-blue-50 border border-blue-200 p-5 rounded-2xl flex items-start gap-4 animate-in fade-in">
                       ...
                       <div>
                         ...
                         <p className="text-sm text-blue-700 mt-1">Gunakan panel <b>Langkah 2</b> di sebelah kiri untuk menyeleksi spesifik ID mana saja yang ingin Anda cetak ulang.</p>
                       </div>
                    </div>
                  )}
                  ...
                  {templateImg && (
                    <div className="bg-white p-4 md:p-6 rounded-2xl shadow-sm border border-slate-200/60 overflow-hidden">
                      <div className="flex justify-between items-center mb-2 border-b border-slate-100 pb-3">
                         ...
                         <div className="flex gap-2">
                           <button onClick={addQrPosition} className="bg-blue-50 text-[#146b99] hover:bg-blue-100 text-[11px] font-bold px-3 py-1.5 rounded-md flex items-center gap-1.5 transition border border-blue-200"><Plus size={14} /> Tambah QR</button>
                           <button onClick={addTextPosition} className="bg-emerald-50 text-emerald-700 hover:bg-emerald-100 text-[11px] font-bold px-3 py-1.5 rounded-md flex items-center gap-1.5 transition border border-emerald-200"><Type size={14} /> Tambah Teks</button>
                         </div>
                      </div>
                      ...
                      <div id="master-template-container" className="relative w-full border-2 border-dashed border-slate-300 rounded-lg overflow-hidden bg-slate-50" style={{ aspectRatio: `${printConfig.widthMm} / ${printConfig.heightMm}` }}>
                        <img src={templateImg} alt="Master Template" className="absolute inset-0 w-full h-full object-contain block pointer-events-none" />
                        
                        {qrPositions.map((pos, index) => (
                          <div key={pos.id} className="absolute bg-white p-1 rounded shadow-xl cursor-move ring-2 ring-transparent hover:ring-blue-500 transition-shadow touch-none z-10 group" style={{ left: `${pos.x}%`, top: `${pos.y}%`, width: `${pos.size}%`, transform: 'translate(-50%, -50%)' }} onMouseDown={(e) => handleDragStart(e, index)} onTouchStart={(e) => handleDragStart(e, index)}>
                             <QRCodeCustom displayValue={`${inputPrefix}XXXXX`} qrPayload={`${window.location.origin}/?verify=${inputPrefix}XXXXX`} size={300} showText={printConfig.embedQrText} />
                             {qrPositions.length > 1 && <button onClick={(e) => { e.stopPropagation(); removeQrPosition(pos.id); }} className="absolute -top-3 -right-3 bg-red-500 text-white rounded-full p-1 opacity-0 group-hover:opacity-100 transition-opacity z-20 hover:scale-110"><X size={12} strokeWidth={3} /></button>}
                        ...
                        {textPositions.map((pos, index) => (
                         <div 
                           ...
                           style={{ left: `${pos.x}%`, top: `${pos.y}%`, transform: 'translate(-50%, -50%)', fontSize: `${pos.size}cqw`, color: 'black', border: '1px dashed transparent' }} 
                           onMouseDown={(e) => handleTextDragStart(e, index)} 
                           onTouchStart={(e) => handleTextDragStart(e, index)} 
                           title="Geser label ID ini"
                         >
                           {inputPrefix}XXXXX
                            <button onClick={(e) => { e.stopPropagation(); removeTextPosition(pos.id); }} className="absolute -top-3 -right-3 bg-red-500 text-white rounded-full p-1 opacity-0 group-hover:opacity-100 transition-opacity z-30 hover:scale-110"><X size={10} strokeWidth={3} /></button>
                            <div className="absolute -top-2.5 -left-2.5 w-5 h-5 bg-emerald-600 border-[2px] border-white rounded-full cursor-nw-resize shadow-md flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity" onMouseDown={(e) => handleTextResizeStart(e, -1, index)} onTouchStart={(e) => handleTextResizeStart(e, -1, index)} />
                  ...
                  <div className="bg-white p-4 md:p-6 rounded-2xl shadow-sm border border-slate-200/60 flex-1">
                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6 gap-3">
                      <div>
                         <h2 className="font-bold text-slate-800 text-lg flex items-center gap-2"><ShieldCheck size={20} className="text-green-500" /> Area Pratinjau <span className="text-xs font-semibold text-gray-600 bg-slate-100 px-2 py-1 rounded ml-1">{generatedQRs.length} Stiker</span></h2>
                      ...
                      {generatedQRs.length > 0 && <button onClick={() => { setGeneratedQRs([]); setPreviewBatchId(null); setSelectedBatchIds([]); }} className="text-red-500 font-bold text-sm hover:bg-red-50 px-3 py-1.5 rounded-lg flex items-center gap-1 transition self-start sm:self-auto shrink-0"><Trash2 size={16} /> Batal & Kosongkan</button>}
                    </div>
                    ...
                    {generatedQRs.length === 0 ? (
                      <div className="flex flex-col items-center justify-center h-64 text-slate-400 border-2 border-dashed border-slate-200 rounded-xl bg-slate-50/50">
                        <Printer size={48} className="mb-3 text-slate-300" />
                      ...
                      <div className="max-h-[700px] overflow-y-auto bg-slate-50 rounded-xl p-2 border border-slate-100 custom-scrollbar">
                        <div className="print-container" style={{ display: 'grid', gridTemplateColumns: `repeat(${printConfig.cols}, minmax(0, 1fr))`, gap: '10px' }}>
                          {generatedQRs.map((item, index) => (
                            <div key={index} className="stiker-item relative bg-white border border-slate-200 overflow-hidden flex items-center justify-center shadow-sm" style={{ aspectRatio: `${printConfig.widthMm} / ${printConfig.heightMm}`, width: '100%', containerType: 'inline-size' }}>
                              {templateImg ? (
                                 <img src={templateImg} alt="Template" className="absolute inset-0 w-full h-full object-contain pointer-events-none" />
                              ) : (
                                 <div className="absolute inset-0 bg-slate-100 flex items-center justify-center text-slate-400 text-xs italic">[Tanpa Template]</div>
                              )}
                              ...
                              {textPositions.map((pos) => (
                                <div key={pos.id} className="absolute whitespace-nowrap text-center" style={{ left: `${pos.x}%`, top: `${pos.y}%`, transform: 'translate(-50%, -50%)', fontSize: `${pos.size}cqw`, color: 'black' }}>
                                  {item.id}
                                </div>
                              ))}
